...
import { CheckCircle, ChevronDown, ChevronUp, ListChecks, FileText, Video } from "lucide-react";
import { dataAnalyticsSteps } from "../../../data/dataAnalyticsRoadmap";

const extractYouTubeID = (url) => {
  const match = url.match(/(?:youtu\.be\/|youtube\.com\/(?:.*v=|v\/|embed\/))([a-zA-Z0-9_-]{11})/);
...
const DataAnalyticsRoadmap = () => {
  ...
  useEffect(() => {
    const storedCompleted = localStorage.getItem("completedDataAnalyticsSteps");
    if (storedCompleted) {
  ...
  const toggleComplete = (index) => {
    ...
    localStorage.setItem("completedDataAnalyticsSteps", JSON.stringify(newCompleted));
  };
  ...
  return (
    <section className="mx-auto py-20 px-6 md:px-16 animate-fade-in dark:bg-gray-900 min-h-screen">
      ...
      <div className="space-y-6">
        {dataAnalyticsSteps.map((step, index) => (
          <div key={index} className="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-xl shadow transition p-6">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`rounded-full w-6 h-6 border-2 ${
                    completed.includes(index) ? "bg-green-500 border-green-500" : "border-gray-400"
                  }`}
                >
                  {completed.includes(index) && <CheckCircle className="text-white w-5 h-5" />}
                </button>
                <h3 className="text-lg font-bold text-gray-800 dark:text-white">
                  {step.title}
                </h3>
              </div>
              {openStep === index ? <ChevronUp className="text-gray-500" /> : <ChevronDown className="text-gray-500" />}
            </div>
            ...
            {openStep === index && (
              <div className="mt-6 ml-8 space-y-6 text-gray-800 dark:text-gray-300">
                ...
                {step.topics && (
                  <div>
                    <h4 className="text-xl font-semibold mb-2 flex">
                      <ListChecks className="w-5 h-5 mr-2 mt-1" /> Topics Covered:
                    </h4>
                ...
                {step.resources && (
                  <div>
                    <h4 className="text-xl font-semibold mb-2 flex">
                      <FileText className="w-5 h-5 mr-2 mt-1" /> Recommended Resources:
                    </h4>
                    <ul className="list-disc space-y-2 pl-5 text-md">
                      ...
                      {step.videos.map((vid, vidIdx) => {
                        ...
                        return videoId ? (
                          <div key={vidIdx} className="w-full">
                            ...
                            <p className="text-center text-md font-medium dark:text-white">{vid.name}</p>
                          </div>
                        ) : null;
                      })}
                    </div>
